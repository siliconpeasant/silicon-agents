...
| 4 综合 | `silicon-crew:soc-synthesis-engineer` | `de/syn/<module>_netlist.v` + `de/syn/timing.rpt` + `de/syn/area.rpt`,WNS ≥ 0 |
...
subagent **必须**把产物放在项目规范目录内,**禁止**自行创建与规范目录功能重复的额外目录:

| 阶段 | 规范目录 | 禁止的额外目录 |
|------|----------|----------------|
| 文档 | `docs/` | — |
| RTL | `de/rtl/` + `de/constraints/` | — |
| 验证 | `dv/tb/` + `dv/sim/` | — |
| 综合 | `de/syn/` | `syn/`, `syn/output/`, `syn/reports/` 等 |

主 Agent 在 spawn subagent 时,必须在 prompt 中显式告知规范目录绝对路径。subagent 完成后,主 Agent 检查产物是否全部落在规范目录内;发现额外目录立即删除。
